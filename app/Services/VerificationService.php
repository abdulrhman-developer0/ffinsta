...
class VerificationService
{
    ...
    public function getCountViaRapidApi(string $username, string $type = 'follower'): ?int
    {
        ...
        $url = "https://{$rapidApiHost}/profile";
        $params = ['username' => $username];

        if (str_contains($rapidApiHost, 'instagram-scraper-api2')) {
            $url = "https://{$rapidApiHost}/v1/info";
            $params = ['username_or_id_or_url' => $username];
        } elseif (str_contains($rapidApiHost, 'instagram-looter2')) {
            $url = "https://{$rapidApiHost}/profile";
            $params = ['username' => $username];
        }
        ...
        if ($response->successful()) {
            $data = $response->json();
            $count = $this->extractCount($data, $type);
            if ($count !== null) {
                return $count;
            }
            throw new Exception("RapidAPI returned success but {$type} count not found in response.");
        }

        throw new Exception("RapidAPI returned status: " . $response->status());
    }

    protected function extractCount(array $data, string $type): ?int
    {
        $keys = $type === 'follower' 
            ? ['follower_count', 'followers', 'edge_followed_by'] 
            : ['following_count', 'following', 'edge_follow', 'follows_count'];

        foreach ($keys as $key) {
            if (isset($data[$key])) {
                if (is_array($data[$key]) && isset($data[$key]['count'])) {
        ...
        foreach ($data as $value) {
            if (is_array($value)) {
                $found = $this->extractCount($value, $type);
                if ($found !== null) {
                    return $found;
                }
            }
    ...
    public function verifyFollow(string $followerUsername, string $targetUsername): bool
    {
        ...
        $url = "https://{$rapidApiHost}/followers";
        $params = ['username' => $targetUsername];

        if (str_contains($rapidApiHost, 'instagram-scraper-api2')) {
            $url = "https://{$rapidApiHost}/v1/followers";
            $params = ['username_or_id_or_url' => $targetUsername];
        }
        ...
        try {
            ...
            if ($response->successful()) {
                $data = $response->json();
                return $this->searchForUsernameInArray($data, $followerUsername);
            }
        } catch (\Exception $e) {
    ...
    protected function searchForUsernameInArray(array $data, string $targetUsername): bool
    {
        foreach ($data as $key => $value) {
            if ($key === 'username' && is_string($value) && strtolower($value) === strtolower($targetUsername)) {
                return true;
            }
            if (is_array($value)) {
                if ($this->searchForUsernameInArray($value, $targetUsername)) {
                    return true;
                }
            }
        }
