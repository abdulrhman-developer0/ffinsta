            ...
            <div class="flex flex-col gap-3">
                @foreach($posts as $post)
                    <div class="card p-3 sm:p-4 flex flex-row items-center gap-4 hover:shadow-md transition-all duration-300 group relative">
                        ...
                        <!-- Actions -->
                        <div class="relative z-20 flex-shrink-0 flex items-center gap-1">
                            <!-- Preview Button -->
                            <a href="{{ route('blog.show', $post->localized_slug) }}" target="_blank" class="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 hover:text-brand-600 dark:text-slate-400 dark:hover:text-brand-400 transition-colors focus:outline-none" title="{{ __('Preview') }}">
