                ...
                <div class="flex flex-col lg:flex-row gap-6">
                    ...
                    <div class="flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-6 overflow-hidden min-w-0">
                        ...
                        <div class="flex flex-col gap-3 w-full sm:w-auto sm:min-w-[220px]">
                            @if($order)
                                {{-- Not claimed yet --}}
                                <form method="POST" action="{{ route('user.earn.claim_and_init', $order) }}" id="claim-form-{{ $cardId }}" target="_blank" onsubmit="if(!this.instagram_account_id.value){alert('{{ __('Please add an account first') }}');return false;} setTimeout(() => window.location.reload(), 3000)">
                                    @csrf
                                    <input type="hidden" name="instagram_account_id" id="selected_account_id_{{ $cardId }}" class="account-id-input" value="{{ $userAccounts->first()?->id ?? '' }}">
                                    <button type="submit" class="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm text-[15px]">
