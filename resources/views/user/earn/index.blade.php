<x-app-layout>
        ...
        <div class="flex overflow-x-auto gap-4 pb-2 hide-scrollbar">
            @forelse($userAccounts as $index => $acc)
                <label class="cursor-pointer flex-shrink-0">
                    <input type="radio" name="selected_account_radio" value="{{ $acc->id }}" class="peer sr-only" {{ $index === 0 ? 'checked' : '' }} onchange="document.querySelectorAll('.account-id-input').forEach(el => el.value = this.value);">
    ...
    @if($userAccounts->isEmpty())
        <div class="card p-12 text-center text-muted mb-8 border border-amber-200 bg-amber-50 dark:border-amber-900/50 dark:bg-amber-900/10">
            <svg class="w-16 h-16 mx-auto mb-4 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <p class="text-lg font-bold text-primary mb-2">{{ __('Instagram Account Required') }}</p>
            <p class="text-sm mb-6">{{ __('Please add your Instagram account first in order to complete tasks.') }}</p>
            <a href="{{ route('user.instagram.create') }}" class="inline-flex items-center gap-2 py-3 px-6 bg-brand-500 hover:bg-brand-600 text-white font-bold rounded-xl transition-all shadow-md shadow-brand-500/20">
